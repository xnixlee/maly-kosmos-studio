export function setupSettings({
  ...
  function render() {
    ...
    $("#settings-editor").innerHTML =
      `<div class="settings-grid"><article class="panel"><span class="tag">ГЕНЕРАЦИЯ ИСТОРИЙ</span><h2 class="settings-title">Модель для историй и персонажей</h2><label>Где создавать текст<select id="text-provider"><option value="mlx" ${c.provider === "mlx" ? "selected" : ""}>На этом Mac · MLX</option><option value="openai" ${c.provider === "openai" ? "selected" : ""}>OpenAI · gpt-5.6-luna</option></select></label><label>Локальная модель<select id="text-model">${m.entries
        .filter((x) => x.kind === "mlx")
        .map(
          (x) =>
        ...
        .join(
          "",
        )}</select></label><label>Глубина рассуждения OpenAI<select id="reasoning">${Object.entries(
        {
